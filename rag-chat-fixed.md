...
// Gemini embedding model: gemini-embedding-001 with 768-d output
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const {
      ...
      system_prompt =
        "You are a helpful assistant. Answer questions based on the provided knowledge base context. If the context doesn't contain relevant information, say so honestly.",
      match_count = 5,
    } = body;
    ...
    if (files && files.length > 0) {
      ...
      if (matchError) {
        ...
        log(
          `Found ${matches.length} relevant chunks (similarity: ${
    ...
    const result = {
      ...
      has_knowledge_base: Boolean(files && files.length > 0),
    };
